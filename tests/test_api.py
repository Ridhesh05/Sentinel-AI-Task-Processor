...
# We patch Redis and DB before importing the app so module-level clients don't error.
with (
...
class TestCreateTask:
    def test_create_task_success(self):
        with (
            patch("main.check_rate_limit", return_value=(True, 9, 60)),
            patch("main.create_task", return_value="123456789"),
        ):
            resp = client.post(
                "/tasks",
    ...
    def test_create_task_empty_input(self):
        with patch("main.check_rate_limit", return_value=(True, 9, 60)):
            resp = client.post(
                "/tasks",
                json={"task_type": "summarize", "input_text": ""},
            )
        assert resp.status_code == 422
...
class TestListTasks:
    ...
    def test_list_tasks_with_data(self):
        sample = {"tasks": [{"id": "1", "task_type": "tag", "status": "QUEUED"}]}
        with patch("main.get_recent_tasks", return_value=sample):
            resp = client.get("/tasks")
        assert resp.status_code == 200
